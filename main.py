...
def OpenAndShowMaze(path):
    ...
    for line in f:
        ...
        if(line[-2] == ']'):
            mline = currline
            labi.append(currline.replace(']', '').replace('[', ''))
            currline = ""
    f.close()

    a = len(mline.replace('|', ' ').split())
    b = int((len(labi)/2) + 1)

    actualMaze = [[Cell() for i in range(a)] for j in range(b)]
    t = 0
    for i in range(b):
        actualMaze[i][0]._moveableL = 1
        actualMaze[i][a-1]._moveableR = 1
    for j in range(a):
        actualMaze[0][j]._moveableU = 1
        actualMaze[b-1][j]._moveableD = 1
    ...
    for i in range(len(labi)):
        for j in range(len(labi[i])):
            if t == 0:
                j2 = int(j/2)
                if labi[i][j] == "|":
                    actualMaze[int(i/2)][j2]._moveableR = 1
                    actualMaze[int(i/2)][j2+1]._moveableL = 1
                else:
                    if labi[i][j] == "S":
                        actualMaze[int(i/2)][j2]._cellType = "S"
                    elif labi[i][j] == "R":
                        actualMaze[int(i/2)][j2]._cellType = "R"
                        actualMaze[int(i/2)][j2]._moveableD = 1
                        actualMaze[int(i/2)][j2]._moveableL = 1
                        actualMaze[int(i/2)][j2]._moveableU = 1
                    elif labi[i][j] == "L":
                        actualMaze[int(i/2)][j2]._cellType = "L"
                        actualMaze[int(i/2)][j2]._moveableD = 1
                        actualMaze[int(i/2)][j2]._moveableR = 1
                        actualMaze[int(i/2)][j2]._moveableU = 1
                    elif labi[i][j] == "U":
                        actualMaze[int(i/2)][j2]._cellType = "U"
                        actualMaze[int(i/2)][j2]._moveableD = 1
                        actualMaze[int(i/2)][j2]._moveableL = 1
                        actualMaze[int(i/2)][j2]._moveableR = 1
                    elif labi[i][j] == "D":
                        actualMaze[int(i/2)][j2]._cellType = "D"
                        actualMaze[int(i/2)][j2]._moveableR = 1
                        actualMaze[int(i/2)][j2]._moveableL = 1
                        actualMaze[int(i/2)][j2]._moveableU = 1
                    elif labi[i][j] == "T":
                        actualMaze[int(i/2)][j2]._cellType = "T"
                    elif labi[i][j] == "E":
                        actualMaze[int(i/2)][j2]._cellType = "E"
                    elif labi[i][j] == "A":
                        actualMaze[int(i/2)][j2]._cellType = "A"

            else:
                if labi[i][j] == "_":
                    actualMaze[int(i/2)][j2]._moveableD = 1
                    actualMaze[int((i/2)+1)][j2]._moveableU = 1
        if t == 0:
            t = 1
        else:
            t = 0
...
def DFSwithTP(corr, form, graph):
    canExit = 0
    visited = [[0]*form[1] for _ in range(form[0])]
    stack = []
    stack.append(corr)
    ...
    while(True):
        for i in range(len(players)):
            if(players[i]._canmove == 0):
                ...
                command = ""
                while command != "w" and command != "s" and command != "d" and \
                      command != "a" and command != "shoot w" and \
                      command != "shoot d" and command != "shoot s" \
                      and command != "shoot a":
                    command = input()
                    cX = players[i]._x
                    cY = players[i]._y
                    if command == "w":
                        if actualMaze[cX][cY]._cellType == "E":
                            if(commToExit == "w"):
                                return i
                        if actualMaze[cX][cY]._moveableU == 0:
                            players[i].set_corr(cX - 1, cY)
                            print("You moved 1 cell up")
                            if actualMaze[cX][cY]._cellType == "S":
                                players[i].set_stun(stunCount)
                                print("You are stunned for " + str(stunCount) + " turns")
                            if actualMaze[cX][cY]._cellType == "A":
                                players[i].set_ammo(3)
                                print("You entred the armory")
                            if actualMaze[cX][cY]._cellType == "T":
                        ...
                        if actualMaze[cX][cY]._cellType == "E":
                            if(commToExit == "s"):
                                return i
                        if actualMaze[cX][cY]._moveableD == 0:
                            players[i].set_corr(cX + 1, cY)
                        ...
                        if actualMaze[cX][cY]._cellType == "E":
                            if(commToExit == "d"):
                                return i
                        if actualMaze[cX][cY]._moveableR == 0:
                            players[i].set_corr(cX, cY + 1)
                        ...
                        if actualMaze[cX][cY]._cellType == "E":
                            if(commToExit == "a"):
                                return i
                        if actualMaze[cX][cY]._moveableL == 0:
                            players[i].set_corr(cX, cY - 1)
